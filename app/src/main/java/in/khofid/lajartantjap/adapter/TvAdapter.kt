...
class TvAdapter(var context: Context, var tvShows: List<TvShow>, var listener: (TvShow) -> Unit): RecyclerView.Adapter<TvViewHolder>(){
    ...
    override fun onBindViewHolder(holder: TvViewHolder, pos: Int) {
        holder.bindItem(tvShows[pos], listener)
    }
}

class TvViewHolder(view: View): RecyclerView.ViewHolder(view){
    fun bindItem(tv: TvShow, listener: (TvShow) -> Unit) {
